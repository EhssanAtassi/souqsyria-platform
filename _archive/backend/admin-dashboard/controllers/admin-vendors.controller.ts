...
export class AdminVendorsController {
  ...
  private async getVendorOrderCount(vendorId: number): Promise<number> {
    ...
    // Join chain: product → product_variants → order_items (no direct product_id on order_items)
    const result = await this.productRepository
      .createQueryBuilder('product')
      .innerJoin('product_variants', 'pv', 'pv.product_id = product.id')
      .innerJoin('order_items', 'oi', 'oi.variant_id = pv.id')
  ...
  private async getVendorFinancials(vendorId: number): Promise<{
    ...
    // Join chain: product → product_variants → order_items → orders
