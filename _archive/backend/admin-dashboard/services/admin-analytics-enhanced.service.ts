  ...
  async getFunnelOverview(query: DateRangeQueryDto): Promise<FunnelOverviewResponseDto> {
    ...
    return this.cacheService.getOrSet(
      ...
      async () => {
        ...
        // Default to last 90 days when no date range specified
        const funnelStartDate = query.startDate ? new Date(query.startDate) : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
        const funnelEndDate = query.endDate ? new Date(query.endDate) : new Date();
  ...
  async getFunnelSteps(query: DateRangeQueryDto): Promise<FunnelStepsResponseDto> {
    ...
    return this.cacheService.getOrSet(
      ...
      async () => {
        ...
        const stepsStartDate = query.startDate ? new Date(query.startDate) : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
        const stepsEndDate = query.endDate ? new Date(query.endDate) : new Date();
        const funnelData = await this.funnelService.getFunnelAnalytics(
          stepsStartDate,
          stepsEndDate,
        );
  ...
  async getDropOffAnalysis(query: DateRangeQueryDto & { stage?: string }): Promise<FunnelDropOffResponseDto> {
    ...
    return this.cacheService.getOrSet(
      ...
      async () => {
        ...
        const dropoffStartDate = query.startDate ? new Date(query.startDate) : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
        const dropoffEndDate = query.endDate ? new Date(query.endDate) : new Date();
        const funnelData = await this.funnelService.getFunnelAnalytics(
          dropoffStartDate,
          dropoffEndDate,
        );
  ...
  async getDeviceFunnels(query: DateRangeQueryDto): Promise<DeviceFunnelResponseDto> {
    ...
    return this.cacheService.getOrSet(
      ...
      async () => {
        ...
        const deviceStartDate = query.startDate ? new Date(query.startDate) : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
        const deviceEndDate = query.endDate ? new Date(query.endDate) : new Date();
        const funnelData = await this.funnelService.getFunnelAnalytics(
          deviceStartDate,
          deviceEndDate,
        );
