...
export class AdminAnalyticsService {
  ...
  async getTopProductsData(query: SalesAnalyticsQueryDto & { limit?: number }): Promise<Array<{
    ...
    // Join chain: OrderItem → variant (ProductVariant) → product (ProductEntity)
    ...
    return currentPeriodProducts.map(r => {
      ...
      return {
        ...
        image: '/assets/images/placeholder-product.jpg', // Images are in ProductImage relation
        trend,
      };
    });
  }
  ...
  async getSalesCategoriesData(query: SalesAnalyticsQueryDto): Promise<Array<{
    ...
    // Join chain: OrderItem → variant → product → category
  ...
  private async calculateRevenue(
    ...
    // For vendor/category filtering, join through variant → product
    if (vendorIds?.length || categoryIds?.length) {
      ...
      if (vendorIds?.length) {
        query = query.andWhere('p.vendor_id IN (:...vendorIds)', { vendorIds });
      }
      if (categoryIds?.length) {
        query = query.andWhere('p.category_id IN (:...categoryIds)', { categoryIds });
      }
  ...
  private async countOrdersInRange(startDate: Date, endDate: Date, vendorIds?: number[]): Promise<number> {
    if (vendorIds?.length) {
      ...
      // Join chain: Order → items → variant → product
      const result = await this.orderRepository
        .createQueryBuilder('o')
        ...
        .andWhere('p.vendor_id IN (:...vendorIds)', { vendorIds })
        .getRawOne();
  ...
  private async getTopProductsBySales(
    ...
    // Join chain: OrderItem → variant → product
    const result = await this.orderItemRepository
      .createQueryBuilder('oi')
      .select([
        'p.id as id',
  ...
  private async getTopVendorsBySales(startDate: Date, endDate: Date, limit: number) {
    // Join chain: OrderItem → variant → product → vendor
    const result = await this.orderItemRepository
      .createQueryBuilder('oi')
      .select([
        'v.id as id',
